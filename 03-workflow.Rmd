...
GitHub is a foundational tool that is central to our data management workflow.  We've described how it can be used as an intermediate solution for hosting data (section \@ref(datahome)) and how we use it to share and automate our reporting workflows (section \@ref(automation)).  Our use of GitHub is in line with our broader philosophy of using open science and its useful to explain some more general concepts about what GitHub can provide to our community. 

Many people describe Git and GitHub synonymously, but we need to distinguish between the two to develop an understanding of the different services each provides.  First, Git is a formal version control software, whereas GitHub is an online platform for sharing code that uses Git.  It's possible to use Git without using GitHub (i.e., using version control on your personal computer) and it's possible to use GitHub without using Git (e.g, using GitHub to share a file).  As expected, using both Git and GitHub together is a preferred approach to leveraging the benefits of each. The relationship between the two is very similar to that of R and RStudio.  Using R by itself is okay, but the value to yourself and others of using the RStudio as a vehicle for R will be greatly enhanced.  The same applies to Git and GitHub. 
...
* Attaching licenses to a project with visible links ot the usage defined under each license
...
GitHub also lowers the barrier to inclusion for engaging others in a project.  Unless you work with a dedicated team of software or web developers, it's very rare that your colleagues will have experience with Git or even know what it is (although this is slowly changing).  This doesn't mean that others are excluded from contributing.  For example, anyone can post issues for a project through the simple web interface provided by GitHub.  Changes to source documents can also be made online that can be tracked through version control without having to use Git on your own (e.g., see our contributing section in \@ref(contrib)).  
