...
Finally, if we wanted to convert this information to an EML metadata file, we can use some tools in the EML R package.  Although there are specific tags and entry methods for each of the attributes, this includes all of the information from the last example.  The methods information can als be entered as a separate file for more long-form documentation. After the file is written with `write_eml()`, it can be validated for accuracy and completeness with `eml_validate()`.  The finished metadata file can be viewed [here](data/my_eml.xml). 
