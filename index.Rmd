...
Uisng an open science ethos, we strongly encourage community collaboration in contributing to how this document evolves.  Please follow the guidelines in section \@ref(contrib) to learn how to contribute and improve this SOP. 

## Contributing to this document {#contrib}

This SOP was created using [bookdown](https://bookdown.org/), which is an approach to creating long-form documents with RMarkdown.  The source code is available on the TBEP GitHub group web page here: <https://github.com/tbep-tech/data-management-sop>.  Each section is a plain .Rmd text file that can be edited or commented to  provide feedback on content.  There are several ways you can contribute to or edit this document.
...
In general, one issue should cover only one suggested change to the document.  However, multiple text edits to the same document can be submitted to the same issue so long as they cover similar topics, e.g., one issue for several suggeested edits to one section.  
